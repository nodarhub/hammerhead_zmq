...
class PointCloudRGB:
    ...
    def read(self, buffer, original_offset=0):
        ...
        self.points = np.frombuffer(buffer, dtype=np.float32, count=num_points * 3,
                                    offset=original_offset + PointCloudRGB.HEADER_SIZE).reshape(num_points, 3)
        self.colors = np.frombuffer(buffer, dtype=np.float32, count=num_points * 3,
    ...
    def write(self, buffer, original_offset):
        ...
        if colors.ndim != 2 or colors.shape[1] != 3:
            print(f"Cannot write point_cloud, the colors array should be a numpy array of size N x 3")
            return
        if len(points) != len(colors):
            print(f"Cannot write point_cloud, the points and colors arrays should be the same length")
