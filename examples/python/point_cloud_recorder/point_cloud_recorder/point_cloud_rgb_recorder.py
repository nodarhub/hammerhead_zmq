...
try:
    from zmq_msgs.point_cloud_rgb import PointCloudRGB
    from zmq_msgs.topic_ports import POINT_CLOUD_RGB_TOPIC
except ImportError:
    sys.path.append(
...
def writePlyAscii(filename, points, colors):
    ...
    with open(filename, "w") as out:
        ...
        for pt, rgb in zip(points, rgb8):
            out.write(f"{pt[0]} {pt[1]} {pt[2]} {int(rgb[0])} {int(rgb[1])} {int(rgb[2])}\n")


def writePlyBinary(filename, points, colors):
    assert len(points) == len(colors), "Points and colors must be the same size"
    rgb8 = (colors * 255).astype(np.uint8)
...
class PointCloudRGBRecorder:
    ...
    def loop_once(self):
        ...
        if self.ascii:
            writePlyAscii(filename, point_cloud.points, point_cloud.colors)
        else:
            writePlyBinary(filename, point_cloud.points, point_cloud.colors)
        return
...
def main():
    ...
    topic = POINT_CLOUD_RGB_TOPIC
    default_output_dir = "point_clouds_rgb"
    if len(sys.argv) < 3:
    ...
    subscriber = PointCloudRGBRecorder(endpoint, output_dir)
    while True:
