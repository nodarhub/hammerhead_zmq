...
[[nodiscard]] DetailsParameters detailsFromSoup(const nodar::zmq::PointCloudSoup &soup) {
    ...
    d.projectionType = soup.projection_type;
...
void reprojectSoupToPoints(const nodar::zmq::PointCloudSoup &soup, int downsample, std::vector<PointXYZRGB> &out) {
    ...
    // TODO: need to double check if the following block migrating the cylindrical projection code is done properly.
    if (soup.projection_type != 0) {
        auto isValid = [](const float *const xyz) -> bool {
        ...
        for (size_t row = 0; row < rows; ++row) {
            for (size_t col = 0; col < cols; ++col, xyz += 3, bgr += bgr_step) {
                ...
                out.emplace_back(xyz[0], xyz[1], xyz[2], r8, g8, b8);
            }
        }
        return;
    }
    ...
    // TODO: To merge with parameter q_with_reverse_t_vec of nodar::reprojectImageTo3D.
    //       The negation is only needed if q is with reversed t_vec convension.
